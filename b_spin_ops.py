...
def Heisenberg_1D_Hamiltonian(args, spin_ops, closed_bcs=True, visualization=False, tol=1e-10):
    ...
    H = 0
    H += sum(-.5* args['h'] * spin_ops['sz'][n] for n in range(N-1)) # Zeeman interaction 
    
    if (chain_type == "XX"):
        H += sum(-.5* args['Jx'] *(spin_ops['sx'][n]*spin_ops['sx'][n+1] 
                                 + spin_ops['sy'][n]*spin_ops['sy'][n+1]) for n in range(N-1))
        if closed_bcs and N>2: 
            H += .5* args['Jx'] *(spin_ops['sx'][N-1]*spin_ops['sx'][0] + spin_ops['sy'][N-1]*spin_ops['sy'][0])
        
    elif (chain_type == "XY"):
        H += sum(-.5* args['Jx']* spin_ops['sx'][n]*spin_ops['sx'][n+1] 
                       -.5* args['Jy'] * spin_ops['sy'][n]*spin_ops['sy'][n+1] for n in range(N-1))
